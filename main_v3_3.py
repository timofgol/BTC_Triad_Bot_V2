...
POLR_THRESHOLD = env_float("POLR_THRESHOLD", 25.0)
    ...
    async with aiohttp.ClientSession() as session:
        while not shutdown_event.is_set():
            ...
            if new_phase == current_phase:
                log_ev.debug(f"No phase change. |POLR|={abs(polr):.1f} vs {POLR_THRESHOLD}")
                continue
...
async def main():
    logger.info("Starting BTC Triad Bot V3.3 (STARTUP-tier compatible)…")
    logger.info(
        "Config:\n"
        ...
        f"  POLR_THRESHOLD      = {POLR_THRESHOLD}\n"
    ...
    async with aiohttp.ClientSession() as session:
        if TG_TEST_ON_START:
            await send_telegram(session, "✅ <b>BTC Triad Bot V3.3</b> started (STARTUP plan compatible).")
